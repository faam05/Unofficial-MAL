...
const CSearch = ({ type = 'mal', setOpenedModal, openedModal = null }) => {
  ...
  const { VITE_MAIN_URL, VITE_STREAM_URL } = import.meta.env
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return await axios(`${VITE_MAIN_URL}/anime?q=${debouncedTerm}`)
    },
  ...
  const {
    ...
    queryFn: async () => {
      if (!debouncedTerm) return null
      return await axios(`${VITE_STREAM_URL}/search?q=${debouncedTerm}`)
    },
