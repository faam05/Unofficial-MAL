...
import { Card, Flex, Spoiler, Tabs, Text, Title, Anchor, Badge, Group, List, Accordion } from '@mantine/core'
...
function Detail() {
  ...
  //   if (!isLoading && !isError) {
  //     let id = []
  //     id.push(data?.title.replace(/[^\w\s]/gi, '').replace(/[" "]/g, '-'))
  //     if (data?.title_synonyms.length > 0) id.push(data?.title_synonyms[0].replace(/[^\w\s]/gi, '').replace(/[" "]/g, '-'))
  //     if (data?.title_english) id.push(data?.title_english?.replace(/[^\w\s]/gi, '').replace(/[" "]/g, '-'))
  //     setIds(id)
  //   }
  // }, [isLoading, isError])
