      ...
      <div className='flex flex-1 flex-col justify-between gap-4'>
          ...
          <div className='flex flex-wrap gap-1'>
            {anime.genres.length > 0 &&
              anime.genres.slice(0, 2).map((genre) => (
                <span
                  key={genre}
                  className='rounded-full bg-blue-600 px-2 py-1 text-[10px] font-bold uppercase text-white'
        ...
        <div className='flex items-center justify-between border-t border-slate-300 bg-slate-200 p-4  pt-3'>
          <div className='flex flex-col'>
            <span className='line-clamp-1 text-xs font-bold group-hover:text-blue-400' style={{ color: anime.coverImage.color }}>
