...
export default function Home() {
  ...
  const queryNow = useQuery({
    queryKey: ['nowAnime'],
    queryFn: async () => {
      const response = await fetch(`${VITE_MAIN_SERVICE}/seasons/now`)
  ...
  const querySchedule = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`${VITE_MAIN_SERVICE}/schedules?filter=${date.toLowerCase()}`)
      if (!response.ok) throw new Error('Network response was not ok')
      const { data } = await response.json()
      return data
    },
  ...
  const queryTop = useQuery({
    queryKey: ['topAnime'],
    queryFn: async () => {
      const response = await fetch(`${VITE_MAIN_SERVICE}/top/anime`)
      if (!response.ok) throw new Error('Network response was not ok')
      const { data } = await response.json()
      return data
    },
